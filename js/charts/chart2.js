const pictorialChartAnimation = function(time) {
const chart = this, dataVariables = this.dataVariables;

	let series = chart.series[0];
  let womanPro = dataVariables.histogramWoman[0][1],
  womanRelax = dataVariables.histogramWoman[1][1],
  manPro = dataVariables.histogramMan[0][1],
  manRelax = dataVariables.histogramMan[1][1];

chart.updatePipeline = chart.updatePipeline || {
  500: function() {
    $('.swiper-title').text('Jak się podzieliliśmy?')
    $('.swiper-title').fadeIn(1000);
  },
  2000: function () {
    $('.swiper-subtitle').text("Na zawodach wyniki wpisało " + series.data[0].y + " Mężczyzn");
    $('.swiper-subtitle').fadeIn(1000);
  },
  3000: function() {
    $('.swiper-label').text("Kobiety nie zostały daleko w tyle, wyniki wrzuciło ich aż " + series.data[1].y);
  ...
  10000: function() {
    $('.swiper-subtitle').text("Zdecydowanie więcej z nas przyszło się zrelaksować, ale było też PRO");
$('.swiper-subtitle').fadeIn(1000);

chart.series[0].remove(false);
chart.addSeries({
    type: 'item',
    name: 'Udział w zawodach',
    itemPadding: 0,
    data: [{
        name: 'Pro',
        y: manPro + womanPro,
        marker: {
            symbol: 'url(./assets/man.svg)'
        },
        color: 'rgb(118,	246,	81)'
      },{
        name: 'Relax',
        y: manRelax + womanRelax,
        marker: {
          symbol: 'url(./assets/man2.svg)'
        },
...
chart.update({
  ...
  13000: function(){
    $('.swiper-label').text("Zarówno wśród kobiet, jak i mężczyzn");
    $('.swiper-label').fadeIn(1000);
  },
  15000: function() {
    chart.series[0].remove(false);
...
export const getChart2Options = function (dataVariables) {
return {
    chart: {
        type: 'pictorial',
        events: {
          load: function() {
            ...
            this.updateChart = pictorialChartAnimation;
          }
        }
    },
  ...
  credits: {
    ...
    yAxis: {
        ...
        max: (function(){
          let all = 0;
          dataVariables.histogramGender.forEach(function(p){
            all += p.y
          })
        }())
    },
    ...
    series: [{
      color: 'rgb(118,	246,	81)',//'rgb(237,	237,	237)',
      name: 'Udział w zawodach',
    data: dataVariables.histogramGender.map(function(p) {
      if (p[0]==="K") {
        p[0] = "Kobiety"
      } else if (p[0]==="M") {
        p[0] = "Mężczyźni"
